/* m2k_step.c — the steppable state machine: relay loops, command-mode
   input, app-IO, poll-set builders, per-state iterators, and the
   public m2k_get_pollfds/m2k_step/m2k_run surface. Split out of
   m2k_api.c (issue #71). */
...
/* online mode relay loop — returns 0=connection lost, 1=+++ escape.
   Non-static: the deprecated m2k_online() in m2k_api.c still calls it. */
int
