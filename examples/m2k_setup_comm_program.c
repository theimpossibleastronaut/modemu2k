/*
 * Compilable example for m2k_setup_comm_program(). The %s in the command
 * string is replaced by modemu2k with the PTY slave device path before
 * sh -c is exec'd, so the comm program ends up talking to the modem-
 * emulator PTY.
 */
#include <stdio.h>
#include <modemu2k.h>
